...
try:
    from tracehunt.__init__ import import_error_test_var # noqa: F401
except ImportError:
    print("Did you run TraceHunt with `python3 tracehunt/tracehunt.py ...`?")
    print("This is an outdated method. Please see https://github.com/Hayatelin/tracehunt for up to date instructions.")
    sys.exit(1)
...
from tracehunt.__init__ import (
    __longname__,
    __shortname__,
    __version__,
...
from tracehunt.result import QueryResult
from tracehunt.notify import QueryNotify
from tracehunt.notify import QueryNotifyPrint
from tracehunt.sites import SitesInformation
from tracehunt import report as th_report
from tracehunt.config import load_config
from colorama import init
from argparse import ArgumentTypeError


class TraceHuntFuturesSession(FuturesSession):
    def request(self, method, url, hooks=None, *args, **kwargs):
        ...
        return super(TraceHuntFuturesSession, self).request(
...
def check_for_parameter(username):
    ...
    if exist it means that tracehunt is looking for more multiple username"""
    return "{?}" in username
...
def run_search(
    ...
    session = TraceHuntFuturesSession(
    ...
    for social_network, net_info in site_data.items():
        ...
        # As WAFs advance and evolve, they will occasionally block TraceHunt and
...
def main():
    ...
    parser.add_argument(
        ...
        dest="html",
        default=None,
        help="[TraceHunt] Write a styled standalone HTML report to FILE.",
    )
    parser.add_argument(
        "--summary",
        action="store_true",
        dest="summary",
        default=False,
        help="[TraceHunt] Print a digital-footprint summary (counts + 0-100 score) per username.",
    )
    parser.add_argument(
        "--config",
        metavar="FILE",
        dest="config",
        default=None,
        help="[TraceHunt] Load default options from a YAML config file.",
    )
    parser.add_argument(
        "--check-update",
        action="store_true",
        dest="check_update",
        default=False,
        help="[TraceHunt] Check GitHub for a newer release (off by default; never phones home unless asked).",
    )
    parser.add_argument(
        "--site",
        action="append",
    ...
    # [TraceHunt] Merge YAML config defaults; explicit CLI flags always win.
    _cfg = load_config(getattr(args, "config", None))
    for _k, _v in _cfg.items():
        if hasattr(args, _k) and getattr(args, _k) in (None, False, [], ""):
            setattr(args, _k, _v)

    # If the user presses CTRL-C, exit gracefully without throwing errors
    signal.signal(signal.SIGINT, handler)

    # [TraceHunt] Update check is OPT-IN (privacy): only runs with --check-update.
    ...
    try:
        if args.local:
            ...
            if not json_file_location:
                # [TraceHunt] Default to the bundled, OFFLINE site database so the
                # tool works out-of-the-box with no network call.
                json_file_location = os.path.join(os.path.dirname(__file__), "resources/data.json")
            if args.json_file:
                # If --json parameter is a number, interpret it as a pull request number
                if args.json_file.isnumeric():
                    pull_number = args.json_file
                    pull_url = f"https://api.github.com/repos/Hayatelin/tracehunt/pulls/{pull_number}"
                    ...
                    json_file_location = f"https://raw.githubusercontent.com/Hayatelin/tracehunt/{head_commit_sha}/tracehunt/resources/data.json"

            sites = SitesInformation(
                data_file_path=json_file_location,
                honor_exclusions=(not args.ignore_exclusions) and bool(args.json_file),
    ...
    for username in all_usernames:
        ...
        # [TraceHunt] Optional HTML report and footprint summary.
        if getattr(args, "html", None):
            _hp = th_report.write_html(username, results, args.html, only_found=not args.print_all)
            print(f"HTML report saved: {_hp}")
        if getattr(args, "summary", False):
            th_report.print_summary(th_report.summarize(username, results))
